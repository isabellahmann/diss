...
class BraTSEnsembleDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # selected_pred = random.choice(ensemble_preds)  # Choose a random prediction
                ...
                # Path to save images
        save_dir = "ensemble_visualizations"
        os.makedirs(save_dir, exist_ok=True)

        selected_pred = ensemble_preds[random.randint(0, ensemble_preds.shape[0] - 1)]
...
if __name__ == "__main__":
    ...
    train_loader = get_ensemble_data_loader(
        batch_size=8,
        split="train",
    ...
    val_loader = get_ensemble_data_loader(
        batch_size=8,
        split="val",
        modality='flair',  # Load only selected modalities
        mask_type='binary',  # Multi-class mask
...
for batch_idx, batch in enumerate(train_loader):
    modality_data, seg_data, patient = batch
    print(f"Modality data shape: {modality_data.shape}, Segmentation data shape: {seg_data.shape}")
    
    # Create a figure with subplots to display 8 images and their corresponding masks
    fig, axes = plt.subplots(8, 2, figsize=(10, 20))  # 8 rows and 2 columns: one for image, one for mask
    ...
    for i in range(modality_data.shape[0]):
        ...
        # Display the image on the left column
        axes[i, 0].imshow(first_image, cmap='gray')
        axes[i, 0].set_title(f"Slice {i+1} - {modality}")
        axes[i, 0].axis('off')  # Hide axes for better visualization

        # Display the segmentation mask on the right column
        axes[i, 1].imshow(first_mask, cmap='jet')
